...
def analyze_cache_control_patterns(js_code: str) -> Dict[str, Any]:
    ...
    if any(
        pattern in js_code
        for pattern in ["validateContentForCaching", "safeExecuteOperation", "fallback", "else.*text.*:", "return.*fallback"]
    ):
    ...
    for scenario_name, value in test_scenarios:
        try:
            ...
            if value is None:
                ...
                is_valid = (
                    len(content_str) > 0
                    and len(content_str.strip()) > 0
                    and len(content_str.strip()) >= 5
                    and content_str.strip().lower() not in ["null", "undefined", "[]", "{}", "none", "empty"]
                )
            ...
            simulations.append(
                {"scenario": scenario_name, "value": str(value), "is_valid": False, "error": str(e), "would_cause_error": True}
            )

    return {"simulations": simulations, "risky_scenarios": len([s for s in simulations if s.get("would_cause_error", False)])}
...
def analyze_file(file_path: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    file_analysis = {"file": file_path, "js_blocks_count": len(js_blocks), "blocks": [], "overall_risk": "low"}
...
def main():
    ...
    for file_path in md_files:
        ...
        if not args.json:
            risk_indicator = {"high": "[HIGH]", "medium": "[MEDIUM]", "low": "[LOW]"}[analysis["overall_risk"]]
        ...
        for analysis in high_risk_files:
            ...
            for block in analysis["blocks"]:
                if block["issues"] or block["risky_scenarios"] > 0:
                    print(
                        f"    Line {block['line']}: {len(block['issues'])} issues, {block['risky_scenarios']} risky scenarios"
                    )
    ...
    files_with_cache_control = sum(
        1 for analysis in all_files_analysis if any(block["has_cache_control"] for block in analysis["blocks"])
    )
