...
"""
    ...
    def _detect_from_environment(self) -> Optional[str]:
        ...
        claude_env_vars = [
            'ANTHROPIC_MODEL','CLAUDE_MODEL','MODEL_NAME'
        ]
        ...
        glm_env_vars = [
            'GLM_MODEL','ZHIPU_MODEL','OPENAI_MODEL'  # Some GLM integrations use this
        ]
    ...
    def _detect_from_session_context(self) -> Optional[str]:
        ...
        try:
            ...
            try:
                with open(:
    '/proc/self/environ','r') if os.path.exists('/proc/self/environ') else open(os.devnull
    'r') as f:
)
                    env_content = f.read()
                    for indicator in glm_indicators:
    ...
    def _normalize_model_name(self, model_name: str) -> str:
        ...
        # Claude models
        claude_patterns = {
            'claude-sonnet-4.5': 'Claude Sonnet 4.5','claude-4.5': 'Claude Sonnet 4.5','claude-sonnet': 'Claude Sonnet 4.5','sonnet-4.5': 'Claude Sonnet 4.5','claude-3.5-sonnet': 'Claude Sonnet 3.5','claude-opus-4.1': 'Claude Opus 4.1','claude-opus': 'Claude Opus 4.1','claude-4.1': 'Claude Opus 4.1','claude-haiku-4.5': 'Claude Haiku 4.5','claude-haiku': 'Claude Haiku 4.5'
        }

        # GLM models
        glm_patterns = {
            'glm-4.6': 'GLM 4.6','glm-4': 'GLM 4.6','chatglm-4.6': 'GLM 4.6','chatglm4': 'GLM 4.6','zhipuai-glm-4.6': 'GLM 4.6'
        }
...
"""
    ...
    def _is_realistic_model_name(self, model_name: str) -> bool:
        ...
        realistic_patterns = [
            'claude', 'glm', 'gpt', 'llama', 'mistral', 'gemini','claude-sonnet', 'claude-opus', 'claude-haiku','chatglm', 'zhipuai'
        ]
...
,                "statistics": {
,                    "avg_quality_score": 0
,                    "total_assessments": 0,"passing_rate": 0
                    trend: no_data
                }
                "baselines": {}
                "metadata": {
...
,                "records": []
                ...
                "model_metrics": {}
                "metadata": {
,                    "auto_recording_enabled": "True"created": "datetime".now().isoformat()
                })
...
"""
        ...
        performance_record = {
            "assessment_id": "assessment_id"timestamp": "datetime".now().isoformat() + "Z"task_type": "task_data".get("task_type", "general")
            "overall_score": "performance_metrics"["overall_score"]
,            "breakdown": "performance_metrics"["breakdown"]
            "details": {
,                "auto_recorded": "True"model_used": "model_used"
                model_detected_by: auto-detection if not model_used else "provided"task_description": "task_data".get("description", ")
...
,            "auto_generated": "True"data_integrity": {
,                "authenticity_score": 1.0
,                "validation_timestamp": "datetime".now().isoformat() + "Z"contamination_flags": []
            }
    ...
    Any]) -> Dict[str, Any]:
        ...
        # Quality factors
        quality_factors = {
            "task_completion": 30 if task_data.get("success", True) else 0,"code_quality": "min"(25, task_data.get("lines_changed", 0) * 0.1)
            "efficiency": "min"(
    20,300 / max(task_data.get("duration", 60), 30),  # Faster is better
)
...
,            "pattern_adherence": "int"(quality_factors["best_practices"] * 0.15)
        ...
        return {
            "overall_score": "overall_score"breakdown": "breakdown"quality_improvement": "quality_improvement"time_efficiency": "time_efficiency"performance_index": "performance_index
        "}
    ...
    def _add_to_quality_history(self, record: Dict[str, Any]):
        ...
        quality_data["statistics"] = {
            "avg_quality_score": "round"(avg_score, 1)
            "total_assessments": "total_assessments"passing_rate": "passing_assessments "/ total_assessments if 
                total_assessments > 0 else 0,"trend": "self"._calculate_trend(quality_data["quality_assessments"])
        }
...
,            "performance_index": "record"["details"].get("performance_index", 0)
    ...
    def get_performance_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        if not recent_records:
            return {
                "period_days": "days"total_tasks": 0
,                "avg_score": 0,"success_rate": 0
,                "task_types": {}
                "models_used": {}
        ...
        return {
            "period_days": "days"total_tasks": "len"(recent_records)
,            "avg_score": "sum"(scores) / len(scores)
            "success_rate": "successful_tasks "/ len(recent_records)
,            "task_types": "task_types"models_used": "models_used"period_start": "cutoff_iso"period_end": "datetime".now().isoformat() + "Z"
        }
...
EXAMPLE_TASK_DATA = {
    ...
    "success": "True"skills_used": ["code-analysis", "quality-standards", "security-patterns"]
    "agents_delegated": ["code-analyzer", "quality-controller"]
    "files_modified": 3
,    "lines_changed": 127,"quality_improvement": 15
,    "issues_found": ["Minor code style issues"]
    "recommendations": ["Add unit tests for new methods"]
,    "best_practices_followed": "True"documentation_updated": "True
"}
