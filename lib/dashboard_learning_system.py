...
class DashboardLearningSystem:
    ...
    def _identify_applicable_contexts(self, task_data: Dict) -> List[str]:
        ...
        if "debugging" in task_type:
            contexts.extend(
                ["debugging_performance_analysis", "data_validation_automation", "quality_improvement_initiatives"]
            )
        ...
        try:
            ...
            return {
                ...
                "historical_success_rate": (
                    insights.get("success_metrics", {}).get("successful_tasks", 0)
                    / max(1, insights.get("success_metrics", {}).get("total_tasks", 1))
                )
                * 100,
            }
            ...
            return {"suggestions": ["Continue with systematic debugging approach"], "confidence": "low", "error": str(e)}
