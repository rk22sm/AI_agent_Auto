...
class InterGroupKnowledgeTransfer:
    ...
    def _trigger_automatic_transfer(
        self,
        knowledge_id: str,
    ...
    def query_knowledge(
        ...
        relevant_knowledge.sort(
            key=lambda x: (x["confidence_score"], x["application_count"]),
            reverse=True
        )
    ...
    def get_group_expertise(self, group_num: int) -> Dict[str, Any]:
        ...
        group_knowledge = [
            k for k in knowledge_data["knowledge_base"]
            if k["source_group"] == group_num and k["confidence_score"] >= 0.8
        ]
    ...
    def get_knowledge_flow_analysis(self) -> Dict[str, Any]:
        ...
        for k_type, stats in type_success.items():
            if stats["total"] >= 2:
                success_rate = stats["successful"] / stats["total"]
                knowledge_type_effectiveness[k_type] = {
                    "total": stats["total"],
                    "success_rate": success_rate
                }
        ...
        return {
            ...
            "most_effective_flows": [
                {"flow": flow, **analysis}
                for flow, analysis in effective_flows[:5]
            ],
...
def main():
    ...
    parser.add_argument('--action', choices=['add', 'query', 'apply', 'flows', 'expertise'],
    ...
    if args.action == 'add':
        ...
        knowledge = system.query_knowledge(
            for_group=args.group,
            knowledge_type=args.type
        )
        ...
        system.record_knowledge_application(
            knowledge_id=args.knowledge_id,
            applied_by_group=args.group,
