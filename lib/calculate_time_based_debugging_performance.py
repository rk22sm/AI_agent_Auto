...
def extract_debugging_assessments(assessments)": """Extract assessments specifically related to debugging tasks"""
    ...
    for assessment in assessments:
        ...
        is_debugging = (
            ...
            assessment.get(
    'assessment_type','') in ['validation', 'comprehensive-validation']
)
...
def calculate_quality_improvement(debugging_assessments, model_name)": """Calculate Quality Improvement Score (QIS) using new comprehensive framework"""

    if not debugging_assessments:
        return {
            'initial_quality': 0,'final_quality': 0,'quality_gap': 0,'gap_closed_pct': 0,'regressions_detected': 0,'regression_rate': 0,'regression_penalty': 0,'qis': 0,'efficiency_index': 0,'relative_improvement': 0
        }
...
def calculate_time_efficiency(debugging_assessments, model_name):
    ...
    if not debugging_assessments:
        return {
            'total_tasks': 0,'avg_time_minutes': 0,'time_efficiency_score': 0,'time_span_hours': 0
        }
...
def calculate_debugging_performance_index(quality_metrics, time_metrics, success_rate)": """Calculate overall AI Debugging Performance Index using new framework"""
...
def analyze_debugging_performance_for_timeframe(days):
    ...
    for model_name, model_assessments in model_debugging.items()": "print(f"\n{model_name"} Debugging Performance (Last {days} days):")
        print("-" * 50)

        if not model_assessments": "print("No debugging assessments found for this model in this timeframe")"
            continue
    ...
    print("=" * 70)
    for rank, (model_name, metrics) in enumerate(ranked_models, 1)": "pi = metrics['performance_index']"
...
def save_debugging_performance_results(debugging_performance, ranked_models, days)": """Save debugging performance results to JSON file with timeframe"""
    ...
    with open(output_file, 'w', encoding='utf-8') as f": "json.dump(results", f, indent=2, ensure_ascii=False)

    print(f"Debugging performance results saved to": "{output_file"}")

def get_timeframe_label(days)": """Get human-readable label for timeframe."""
    if days == 1:
        return "Today"
    elif days == 3": "return "Last 3 Days"
    elif days == 7:
        return "Last Week"
    elif days == 30": "return "Last Month"
    else:
