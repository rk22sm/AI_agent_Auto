...
class TaskQueue:
    ...
    def _read_queue(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"Error: Malformed JSON in {self.queue_file}: {e}", file=sys.stderr)
            return []
    ...
    def update_task_status(
        ...
        for task in queue:
            if task['task_id'] == task_id:
                ...
                # Update timestamps
                if status == self.STATUS_RUNNING and old_status == self.STATUS_PENDING:
    ...
    def list_tasks(self, status: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if status:
            queue = [task for task in queue if task['status'] == status]
    ...
    try:
        if args.action == 'add':
            ...
            if task:
                ...
                print(json.dumps({'message': 'No pending tasks'}, indent=2))
