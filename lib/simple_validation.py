...
def validate_claude_plugin(plugin_dir="D:/Git/Werapol/AutonomousAgent"):
    ...
    try:
        ...
        if missing_fields: issues.append(f"Missing required fields: {missing_fields}")
        ...
        if len(description) > 200: warnings.append(f"Description too long: {len(description)} chars (max 200)")
        ...
        for skill_dir in skills_dir.iterdir():
            if skill_dir.is_dir():
                ...
                if skill_file.exists():
                    ...
                    try:
                        ...
                        if content.startswith("---"):
                            try:
                                ...
                                if "name" not in frontmatter_data: issues.append(f"Skill missing name: {skill_file}")
                                if "description" not in frontmatter_data:
                                    issues.append(
                    ...
                    except UnicodeDecodeError: issues.append(f"Invalid file encoding: {skill_file}")
    ...
    if commands_dir.exists():
        for cmd_file in commands_dir.glob("*.md"):
            ...
            try:
                ...
                if len(content.strip()) < 50: warnings.append(f"Command file too short: {cmd_file}")

            except UnicodeDecodeError: issues.append(f"Invalid file encoding: {cmd_file}")
    ...
    if lib_dir.exists():
        for lib_file in lib_dir.glob("*.py"):
            ...
            try:
                ...
                except SyntaxError as e: issues.append(f"Python syntax error in {lib_file}: {e}")

            except UnicodeDecodeError: issues.append(f"Invalid file encoding: {lib_file}")
