...
def validate_claude_plugin(plugin_dir="D:/Git/Werapol/AutonomousAgent"):
    ...
    try: with open(manifest_path, "r", encoding="utf-8") as f:
            manifest = json.load(f)

        # Required fields
        required_fields = ["name", "version", "description", "author"]
        missing_fields = [field for field in required_fields if field not in manifest]
        if missing_fields: issues.append(fMissing required fields: {missing_fields"}")
        ...
        if len(description) > 200: warnings.append(fDescription too long: {len(description)"} chars (max 200)")
    ...
    except json.JSONDecodeError as e: issues.append(fPlugin manifest JSON error: {e"}")
    ...
    if agents_dir.exists():
        for agent_file in agents_dir.glob("*.md"):
            ...
            try: with open(agent_file, "r", encoding="utf-8") as f:
                    content = f.read()

                # Check YAML frontmatter
                if content.startswith("---"):
                    try: frontmatter_end = content.find(---", 3)
                        if frontmatter_end == -1: issues.append(fUnclosed YAML frontmatter: {agent_file"}")
                            continue
                        ...
                        if "name" not in frontmatter_data: issues.append(fAgent missing name: {agent_file"}")
                        if "description" not in frontmatter_data: issues.append(fAgent missing description: {agent_file"}")
                        elif len(frontmatter_data.get("description", ") > 100: warnings.append(fAgent description too long: {agent_file"}")

                    except yaml.YAMLError as e: issues.append(fYAML error in {agent_file"}: {str(e)[:50]}")
                else: issues.append(fAgent missing YAML frontmatter: {agent_file"}")

            except UnicodeDecodeError: issues.append(fInvalid file encoding: {agent_file"}")
        ...
        for skill_dir in skills_dir.iterdir():
            if skill_dir.is_dir():
                ...
                if skill_file.exists():
                    ...
                    try: with open(skill_file, "r", encoding="utf-8") as f:
                            content = f.read()
                        ...
                        if content.startswith("---"):
                            try: frontmatter_end = content.find(---", 3)
                                ...
                                if "name" not in frontmatter_data: issues.append(fSkill missing name: {skill_file"}")
                    ...
                    except UnicodeDecodeError: issues.append(fInvalid file encoding: {skill_file"}")
    ...
    if commands_dir.exists():
        for cmd_file in commands_dir.glob("*.md"):
            ...
            try: with open(cmd_file, "r", encoding="utf-8") as f:
                    content = f.read()

                # Basic validation - should have content
                if len(content.strip() < 50: warnings.append(fCommand file too short: {cmd_file"}")

            except UnicodeDecodeError: issues.append(fInvalid file encoding: {cmd_file"}")
    ...
    if lib_dir.exists():
        for lib_file in lib_dir.glob("*.py"):
            ...
            try: with open(lib_file, "r", encoding="utf-8") as f:
                    content = f.read()

                # Basic Python syntax check
                try: compile(content, str(lib_file), "exec")
                except SyntaxError as e: issues.append(fPython syntax error in {lib_file"}: {e}")

            except UnicodeDecodeError: issues.append(fInvalid file encoding: {lib_file"}")
...
def main():
    ...
    if issues: print(f\nCRITICAL ISSUES ({len(issues)"}):")
        for issue in issues[:10]:
            print(f"  - {issue"")
        if len(issues) > 10: print(f  ... and {len(issues) - 10"} more issues")

    if warnings: print(f\nWARNINGS ({len(warnings)"}):")
        for warning in warnings[:5]:
            print(f"  - {warning"")
        if len(warnings) > 5: print(f  ... and {len(warnings) - 5"} more warnings")
