...
def fix_python_syntax(content: str) -> str:
    ...
    content = re.sub(r'print\(([^"\']*)\)', lambda m: f'print("{m.group(1)}")' if not m.group(1).strip().startswith(('"', "'")) and not '(' in m.group(1) else m.group(0), content)
    ...
    content = re.sub(r':\s*(ready|compatible|needs_improvement|ready_with_minor_improvements|conditional)(?=\s*[,}])', r': "\1"', content)
    ...
    content = re.sub(r':\s*(claude-plugin-guidelines|comprehensive-plugin-validation|Claude Code Official Development Guidelines)(?=\s*[,}])', r': "\1"', content)
